f <- read.table("f.txt")
colnames(f) <- c("месяц", "Курс евро")
f
mediumlevel <- mean(f[, 2])
mediumlevel
result <- numeric()
for (i in 1:length(f[, 2])) {
  if (f[i, 2] > mediumlevel)
    result <- c(result, 1)
  if (f[i, 2] < mediumlevel)
    result <- c(result, 0)
}
...
numberofseries
leftpart <- floor((length(result) + 1) / 2 - 1.96 * sqrt(length(result) - 1) / 2)
leftpart
rightpart <- ceiling((length(result) + 1) / 2 + 1.96 * sqrt(length(result) - 1) / 2)
rightpart
...
smoothed <-
  c(smoothed, (5 * y[length(f[, 1]) - 1] + 2 * y[length(f[, 1]) - 2] - y[length(f[, 1]) - 3]) / 6)
smoothed
t <- numeric()
var <- ((-1) * as.integer(length(y)) + 1)
for (i in 1:length(y)) {
  t <- c(t, var)
  var <- var + 2
}
...
f[, 4] <- t * t
f[, 5] <- f[, 2] * f[, 3]
colnames(f) <- c("месяц", "курс евро", "t", "t^2", "y*t")
f
...
legend(
  x = "topleft",
  y = 0,
  legend = c("исходный ряд", "сглаженный ряд", "ан. выровн. ряд"),
  lty = c(1, 1, 1),
  lwd = c(2.5, 2.5, 2.5),
  col = c("blue", "red", "green")
)